...
import { FiPrinter, FiSettings, FiArrowLeft, FiChevronRight, FiGrid, FiDownload, FiCheckCircle, FiTrash2, FiUpload, FiFileText } from 'react-icons/fi';
...
const BadgeRenderer = ({ event, data, qrCode, onRendered }) => {
        ...
        <Layer>
          ...
          {elements.map((el) => {
             ...
             if (el.type === 'text' || (el.isDynamic && (el.type === 'NAME' || el.type === 'FIELD'))) {
                ...
                if (el.isDynamic) {
                  if (el.type === 'NAME') {
                    text = data.name || data.Name || data.Nom || data.fullname || data.Fullname || data['Nom Complet'] || el.text || '';
                  }
                  if (el.type === 'FIELD' && el.fieldKey) {
                    text = data[el.fieldKey] || el.text || '-';
                  }
                }
                return (
                  <Text
...
export default function BadgeExcelPage({ params }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [evRes, tempRes] = await Promise.all([
          fetch(`/api/events/${id}`),
          fetch(`/api/events/${id}/badge-template`)
        ]);
        
        const [evData, tempData] = await Promise.all([
          evRes.json(),
          tempRes.json()
        ]);
  ...
  const handleSaveAndPrint = async (designData) => {
  ...
  const createPDF = () => {
    ...
    doc.save(`badges_excel_${event.name.replace(/\s+/g, '_')}.pdf`);
        ...
        <div className="flex items-center gap-6">
           ...
           <div>
              <div className="flex items-center gap-3 mb-1">
                 <h2 className="text-3xl font-black text-gray-900 tracking-tight">Badges Automatiques (Excel)</h2>
                 <span className="px-3 py-1 bg-green-500/10 text-green-600 text-[10px] font-black uppercase rounded-full tracking-widest">Mode Batch</span>
              </div>
            ...
            <div className="flex items-center justify-between mb-4">
               <div className="flex items-center gap-3">
                  ...
                  <div>
                     ...
                     <p className="text-[10px] text-gray-400 font-bold uppercase">Traitement de la ligne {currentIdx + 1} sur {excelData.length}</p>
                  </div>
               </div>
